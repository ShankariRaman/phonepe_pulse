    ...
    if Type == "Users":
        ...
        with col1:
            ...
            if Year == 2022 and Quarter in [2, 3, 4]:
                ...
                mycursor.execute(
                    f"select brands, sum(count) as Total_Count, avg(percentage)*100 as Avg_Percentage from agg_users where year = {Year} and quarter = {Quarter} group by brands order by Total_Count desc limit 10")
        ...
        with col3:
            st.markdown("### :violet[Pincode]")
            mycursor.execute(
                f"select Districts, sum(Registered_Users) as Total_Users from top_user1 where year = {Year} and quarter = {Quarter} group by Districts order by Total_Users desc limit 10")
